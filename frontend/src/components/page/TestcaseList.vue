	...
	<div class="table">
		...
		<div class="container">
			...
			<div class="pagination">
				<el-pagination background @current-change="handleCurrentChange" @size-change="handleSizeChange" :page-sizes="[4, 5, 8, 10, 20]"
			...
			<el-form ref="form" :model="form" label-width="120px">
				<el-form-item label="运行环境">
					...
					<el-select v-model="env_id" clearable placeholder="请选择">
						<el-option v-for="item in envs_id_names" :key="item.id" :label="item.name" :value="item.id">
						</el-option>
					</el-select>
			...
			confirmRun() {
				run_by_testcase(this.id, this.env_id)
					.then(response => {
						...
						this.$router.push({
							path: `/reports_view/${response.data.id}`
						})
					})
					.catch(error => {
						this.$message.error('服务器错误');
